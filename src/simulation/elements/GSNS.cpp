...
static int update(UPDATE_FUNC_ARGS)
{
	...
	if ((parts[i].tmp == 0 && (sim->gravIn.mass[Vec2{ x, y } / CELL] >= parts[i].temp - 273.15f) || (parts[i].tmp == 2 && (sim->gravIn.mass[Vec2{ x, y } / CELL]) < parts[i].temp - 273.15f)))
	{
		parts[i].life = 0;
	...
	for (auto  rx = -1; rx <= 1; rx++)
		for (auto  ry = -1; ry <= 1; ry++)
			if (rx || ry)
			{
				...
				{
					float photonWl = sim->gravIn.mass[Vec2{ x, y } / CELL];
					int nx, ny;
					nx = x + rx;
					ny = y + ry;
					while (TYP(r) == PT_FILT)
				...
				else if (parts[i].tmp == 3) //Deserialisation 
				{
					...
					{
					  sim->gravIn.mass[Vec2{ x, y } / CELL]= (float)(parts[ID(r)].ctype - 0x10000000);
					}
				}
			}
	return 0;
